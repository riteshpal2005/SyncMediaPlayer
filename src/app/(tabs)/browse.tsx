...
import { FolderOpen, Search, Video, Music, Play } from 'lucide-react-native';
...
export default function BrowseScreen() {
  ...
  return (
    <ScrollView className="flex-1 px-4 py-8" contentContainerStyle={{ alignItems: 'center' }}>
      ...
      <FolderOpen size={80} color={iconColor} />
        ...
        <View className="mt-12 w-full max-w-md bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700">
          <View className="flex-row items-center mb-4">
            {selectedFile.mimeType?.startsWith('video') ? (
              <Video size={24} color={isDark ? '#3b82f6' : '#2563eb'} />
            ) : (
              <Music size={24} color={isDark ? '#3b82f6' : '#2563eb'} />
            )}
          ...
          <View className="mt-6 flex-row justify-end">
            ...
            >
              <Play size={18} color={isDark ? 'white' : 'black'} />
